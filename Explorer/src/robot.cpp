...
namespace rc
{
    ...
    void Robot::goto_target(std::vector<Eigen::Vector2f> current_line, AbstractGraphicViewer *viewer)
    {
        if(has_target() && abs(adv_tychon) < 0.1f && abs(rot_tychon) < 0.01f)
        {
            Eigen::Vector3f target = current_target.get_coords();
            auto [adv, rot, side] = dynamic_window.update(current_target.get_coords(), current_line, get_current_advance_speed(), get_current_rot_speed(), viewer);
            ...
            try{ omnirobot_proxy->setSpeedBase(0, adv_tychon, rot_tychon); }
            catch(const Ice::Exception &e){ std::cout << e.what() << "Error connecting to omnirobot" << std::endl;}
        }
    }

    void Robot::set_adv_tychon(float adv)
    {
        adv_tychon = adv;
    }

    void Robot::set_rot_tychon(float rot)
    {
        rot_tychon = rot;
    }
    ...
    {
        if(rot_tychon > 0.1f)
            return Eigen::Vector3f{0.f, 0.f, rot_tychon};

        Eigen::Transform<float, 3, Eigen::Affine> tf = get_tf_cam_to_base();
        Eigen::Vector3f dist = tf * get_camera_target_coordinates();
        dist[2] = 0.f;  // dismiss pure rotation here
        dist = dist.normalized() * (dist.norm() - min_distance_to_target);  // set target coordinates before the real target
        return dist;
    }

    Eigen::Vector3f Robot::get_robot_target_coordinates(target objetivo)
    {
        Eigen::Transform<float, 3, Eigen::Affine> tf = get_tf_cam_to_base();
        Eigen::Vector3f dist = tf * objetivo.get_coords();
        dist[2] = 0.f;  // dismiss pure rotation here
        dist = dist.normalized() * (dist.norm() - min_distance_to_target);  // set target coordinates before the real target
        return dist;
    }
   ...
   /*Eigen::Vector3f Robot::get_camera_target_coordinates() const
    ...
    Eigen::Vector3f Robot::get_camera_target_coordinates() const
    {
        return current_target.get_coords();
    }
   ...
   /*void Robot::set_current_targeto(const RoboCompYoloObjects::TBox &target)
    {
        current_target = target;
        has_target_flag = true;
    }*/
    void Robot::set_current_targeto(const target &target)
    {
        current_target = target;
        has_target_flag = true;
    }
    ...
    {
        return atan2(current_target.get_coords().x(), current_target.get_coords().y());
    }
    ...
    target Robot::get_current_target() const
    {
        return current_target;
    }
...
//                                                       Eigen::AngleAxisf(camera_pan_angle, Eigen::Vector3f::UnitZ());
    ...
    void Robot::print()
    {
        ...
        if(has_target_flag)
        {
            ...
            std::cout << "      Type: " << current_target.get_type() << std::endl;
