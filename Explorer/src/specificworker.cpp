...
void SpecificWorker::compute()
{
    ...
    /// YOLO
    RoboCompYoloObjects::TObjects objects_yolo = yolo_detect_objects(top_rgb_frame);
    // detect doors and print them using the methods from door_detection class
    std::vector<DoorDetector::Door> doors = door_detector.detect(omni_lines, viewer);
    
    //door_detector.draw_doors(doors, viewer);

    //create a list of target with the doors detected and the objects detected by yolo
    std::vector<target> targets;
    target targeto;
    auto targets1 = targeto.create_target_list(doors);
    auto targets2 = targeto.create_target_list(objects_yolo);
    targets.insert(targets.end(), targets1.begin(), targets1.end());
    ...
    //draw_objects_on_2dview(targets, RoboCompYoloObjects::TBox());

    // state machine to activate basic behaviours. Returns a target_coordinates vector
    state_machinebro.update(targets,current_line,robot,viewer);
